...
> **⚠️ Note on Server Deployment**: If you plan to run this on a server (e.g., Debian/Ubuntu) rather than your personal computer, DO NOT use the default `npm run shared:start` command as it provides no isolation. Please strictly follow the [Docker Deployment Guide](./DEPLOY_DOCKER.md) to run the gateway and Codex engine under `supervisor`. The current Docker setup bind-mounts your host repository into the container at `/app`, so container-side code changes and AI actions directly affect the host project files.
...
For server deployment (e.g., Debian/Ubuntu) rather than personal computer, use Docker:
...
# Edit .codex/config.toml with your API settings
